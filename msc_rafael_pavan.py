...
#         sep.ext_grid['vm_pu'] = 1.05
        ...
        if algorithm == 'nr':
            ...
            inicio = time.time()
            pp.runpp(sep,algorithm=algorithm,numba = True, init = 'results', tolerance_mva = 1e-8,trafo_model='pi')
            fim = time.time()
...
def fluxo_de_pot(grupo, sep):
    ...
    for linha in range(grupo.shape[0]):
        ...
        grupo[linha,-6] = (sep.res_line['pl_mw'].sum()/100 + sep.res_trafo['pl_mw'].sum()/100) + 0*np.sum((1-sep.res_bus['vm_pu'].values)**2) #func_objetivo(vbus,theta,matrizg,relatorio=False)
...
#         enxame_fit[linha,0:len(sep14.gen)+1] = valor_inicial[0:len(sep14.gen)+1] + (1.06-0.94) *(np.random.randn(len(sep14.gen)+1))*step

#         enxame_fit[linha,0:len(sep14.gen)+1][
#         enxame_fit[linha,0:len(sep14.gen)+1]>1.06] = 1.06

#         enxame_fit[linha,0:len(sep14.gen)+1][
#         enxame_fit[linha,0:len(sep14.gen)+1]<0.94] = 0.94

#         enxame_fit[linha,len(sep14.gen)+1:len(sep14.gen)+1+n_tap] = valor_inicial[len(sep14.gen)+1:len(sep14.gen)+1+n_tap] + (1.1-0.9) * (np.random.randn(n_tap))*step


#         enxame_fit[linha,len(sep14.gen)+1:len(sep14.gen)+1+n_tap][
#         enxame_fit[linha,len(sep14.gen)+1:len(sep14.gen)+1+n_tap]<0.9] = 0.9

#         enxame_fit[linha,len(sep14.gen)+1:len(sep14.gen)+1+n_tap][
#         enxame_fit[linha,len(sep14.gen)+1:len(sep14.gen)+1+n_tap]>1.1] = 1.1
...
#         best_particles[:,-6:] = (fitness(best_particles,zeta,psi,sigma,omega))[:,-6:]
        
#         if global_best[-4]<10 and global_best[-4]>1e-6:
#             global_best = fluxo_de_pot_q(global_best.reshape(1,-1),sep)[0]
#             global_best = fitness(global_best.reshape(1,-1),zeta,psi,0,0)[0]
            
            
#             print('entrou desgraca')

#             best_particles[np.argsort(np.copy(best_particles[:, -1]))][0,:]=global_best.copy()
#             enxame_fit[np.argsort(np.copy(best_particles[:, -1]))][0,:]=global_best.copy()
